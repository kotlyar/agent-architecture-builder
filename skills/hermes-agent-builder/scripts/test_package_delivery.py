...
class PackageDeliveryTest(unittest.TestCase):
    def make_kit(self, root: Path, *, status: str = "готов_к_реализации") -> Path:
        kit = root / "source"
        for name in MODULE.REQUIRED_FILES:
            path = kit / name
            path.parent.mkdir(parents=True, exist_ok=True)
            path.write_text("проверка\n", encoding="utf-8")
        manifest = {
            "schema_version": 1,
        ...
        (profile_root / "profile.md").write_text("профиль\n", encoding="utf-8")
        ...
        for name in (
            ...
            (distribution / name).write_text("проверка\n", encoding="utf-8")
        return kit
    ...
    def test_draft_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            kit = self.make_kit(root, status="черновик")
